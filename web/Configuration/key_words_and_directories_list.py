
subjects_path = '/Applications/freesurfer/subjects/'
projects_paths = './Logs/list_projects_paths.txt'

json_project_names_path = './Logs/exist_projects.json'
list_projects_paths = './Logs/list_projects_paths.txt'

error_log_path = './Logs/error_logs.txt'
event_log_path = './Logs/event_logs.txt'

preprocess_script_path = '/Applications/freesurfer/Startup_freesurfer.sh'
view_script_path = '/Applications/freesurfer/Startup_freeview.sh'

statistic_file = 'aseg.stats'
